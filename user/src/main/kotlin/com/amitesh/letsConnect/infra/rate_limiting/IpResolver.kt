package com.amitesh.letsConnect.infra.rate_limiting

import com.amitesh.letsConnect.infra.config.NginxConfig
...
class IpResolver (
    ...
    fun getClientIp(request: HttpServletRequest): String{
        ...
        val clientIp = extractFromReaIp(request, remoteAddr)
        if (clientIp == null){
    ...
    private fun extractFromReaIp(
        request: HttpServletRequest,
        proxyIp: String
    ): String? {
        return request.getHeader("X-Real_IP")?.let { header ->
    ...
    private fun validateAndNormalizeIp(ip: String, headerName: String, proxyIp: String): String? {
        ...
        return try {
            val inetAddr = when{
                ...
                trimmedIp.matches(Regex("\\d+\\.\\d+\\.\\d+\\.\\d+")) -> Inet6Address.getByName(trimmedIp) // Regular expression Ip before addressess
                else -> {
                    logger.warn("Invaid IP format in $headerName: $trimmedIp from proxy $proxyIp")
                    return null
                }
            }
            ...
            logger.warn("Invaid IP format in $headerName: $trimmedIp from proxy $proxyIp", e)
            null
        }
    }
